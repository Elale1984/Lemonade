...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        //Utilizing when condition for lemonadeState to do the approprate tasks when the
        //given state is present
        when(lemonadeState) {
            SELECT -> {
                lemonadeState = SQUEEZE
                lemonSize = lemonTree.pick() // get the size from the lemonTree.Pick returns random int
                squeezeCount = 0
            }
            SQUEEZE -> {
                squeezeCount++
            ...
            SELECT -> {
                ...
                lemonImage?.setImageDrawable(
                    ContextCompat.getDrawable(
                        applicationContext, R.drawable.lemon_tree
                    )
                )
            }
            SQUEEZE -> {
                ...
                lemonImage?.setImageDrawable(
                    ContextCompat.getDrawable(
                        applicationContext, R.drawable.lemon_squeeze
                    )
                )
            }
            DRINK -> {
                ...
                lemonImage?.setImageDrawable(
                    ContextCompat.getDrawable(
                        applicationContext, R.drawable.lemon_drink
                    )
                )
            }
        RESTART -> {
            ...
            lemonImage?.setImageDrawable(
                ContextCompat.getDrawable(
                    applicationContext, R.drawable.lemon_restart
                )
            )
        }
        }
